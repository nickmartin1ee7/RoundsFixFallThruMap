...
4. `zip` is required. If unavailable, run `nix-shell -p zip --run './build-release.sh'`.
...
- `install-local.sh --plugins /path/to/BepInEx/plugins` — copies the pre-built Release DLL (`RoundsFixFallThruMap/bin/Release/netstandard2.1/RoundsFixFallThruMap.dll`) into a local BepInEx plugins directory. Requires `./build-release.sh` to have been run first.
- `uninstall-local.sh --plugins /path/to/BepInEx/plugins` — removes the DLL from a local BepInEx plugins directory.
